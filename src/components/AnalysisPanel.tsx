...
export const AnalysisPanel: React.FC<AnalysisPanelProps> = ({ play, t, onSeekBeat }) => {
  const [showGlossaryModal, setShowGlossaryModal] = useState(false);
  const activeIdx = activeBeatIndex(play.beats, t);
  const activeBeat = play.beats[activeIdx] || play.beats[0];

  const isDefensiveCoach =
    play.coach === 'saleh' ||
    play.coach === 'macdonald' ||
    play.coach === 'fangio' ||
    play.coach === 'flores' ||
    play.coach === 'belichick' ||
    play.coach === 'minter' ||
    play.coach === 'ryans' ||
    play.coach === 'shula' ||
    play.coach === 'spagnuolo' ||
    play.coach === 'quinn' ||
    play.coach === 'bowles';
